...
class _SingleDeviceWrapper(nn.Module):
    """Mirrors DataParallel's .module attribute for single-device (MPS/CPU) runs."""
...
# RGB colour per class index. Add rows as the model gains more output classes.
_CLASS_PALETTE = np.array([
    [220,  50,  50],   # 0 – vehicles       (red)
    [ 50, 200,  50],   # 1 – drivable area  (green)
    [ 50, 100, 220],   # 2 – pedestrians    (blue)
    [220, 200,  50],   # 3 – cyclists       (yellow)
    [200,  50, 200],   # 4 – traffic cone   (magenta)
    [ 50, 210, 210],   # 5 – static objects (cyan)
], dtype=np.float32) / 255.0


def save_bev_viz(seg_bev_e, seg_bev_g, valid_bev_g=None, drivable_bev_g=None, out_dir='outputs'):
    """Write outputs/latest_bev_grid.jpg (overwritten every call).

    Layout: [prediction panel | GT panel] side-by-side.
    Background: dark teal = drivable surface, near-black = off-road.
    Foreground: class channels composited from _CLASS_PALETTE (e.g. red = vehicles).
    """
    os.makedirs(out_dir, exist_ok=True)

    pred = torch.sigmoid(seg_bev_e[0]).detach().cpu().clamp(0.0, 1.0).numpy()  # (C, Z, X)
    gt   = seg_bev_g[0].detach().cpu().clamp(0.0, 1.0).numpy()                  # (C, Z, X)

    C, Z, X = pred.shape
    palette = _CLASS_PALETTE[:C]  # (C, 3)

    def blend(probs):
        canvas = np.zeros((Z, X, 3), dtype=np.float32)
        for c in range(C):
            canvas += probs[c, :, :, None] * palette[c]
        return canvas.clip(0.0, 1.0)

    # Drivable-area background: teal where drivable=1, near-black where drivable=0.
    if drivable_bev_g is not None:
        drive = drivable_bev_g[0, 0].detach().cpu().numpy().astype(np.float32)  # (Z, X)
        bg = np.stack([
            0.06 + drive * 0.08,   # R channel
            0.12 + drive * 0.28,   # G channel (teal highlight)
            0.12 + drive * 0.24,   # B channel
        ], axis=-1)  # (Z, X, 3)
    else:
        bg = np.zeros((Z, X, 3), dtype=np.float32)

    # Overlay segmentation on the background (background dimmed where seg is active).
    seg_pred = blend(pred)
    seg_gt   = blend(gt)
    pred_rgb = np.clip(bg * (1.0 - seg_pred.max(axis=-1, keepdims=True)) + seg_pred, 0.0, 1.0)
    gt_rgb   = np.clip(bg * (1.0 - seg_gt.max(axis=-1, keepdims=True))   + seg_gt,   0.0, 1.0)

    # Black out don't-care voxels when the valid mask is provided
    if valid_bev_g is not None:
        vmask = valid_bev_g[0, 0].detach().cpu().numpy()[:, :, None]
        pred_rgb *= vmask
        gt_rgb   *= vmask

    # Flip Z so forward (low index) appears at top of image
    pred_rgb = pred_rgb[::-1].copy()
    gt_rgb   = gt_rgb[::-1].copy()

    sep = np.ones((Z, 2, 3), dtype=np.float32)
    composite = np.concatenate([pred_rgb, sep, gt_rgb], axis=1)

    rgb_np = (composite * 255).clip(0, 255).astype(np.uint8)
    Image.fromarray(rgb_np, mode='RGB').save(
        os.path.join(out_dir, 'latest_bev_grid.jpg'), quality=95
    )
...
def run_model(model, loss_fn, d, device=None, sw=None):
    metrics = {}
    total_loss = torch.tensor(0.0, requires_grad=True).to(device)

    imgs, rots, trans, intrins, pts0, extra0, pts, extra, lrtlist_velo, vislist, tidlist, scorelist, seg_bev_g, valid_bev_g, center_bev_g, offset_bev_g, radar_data, egopose, drivable_bev_g = d
    ...
    egopose = egopose[:,0]
    drivable_bev_g = drivable_bev_g[:,0]
    ...
    # One-shot shape diagnostic — prints once per process then stays silent.
    if not getattr(run_model, '_shapes_printed', False):
        C_pred = seg_bev_e.shape[1]
        C_gt   = seg_bev_g.shape[1]
        print(f'\n[shape check]')
        print(f'  seg_bev_e (model prediction) : {tuple(seg_bev_e.shape)}  — {C_pred} class channel(s)')
        print(f'  seg_bev_g (dataset GT)        : {tuple(seg_bev_g.shape)}  — {C_gt} class channel(s)')
        print(f'  valid_bev_g (BEV valid mask)  : {tuple(valid_bev_g.shape)}')
        if C_pred == C_gt:
            print(f'  channel count MATCH: C={C_pred}  (model decoder n_classes == dataset GT channels)\n')
        else:
            print(f'  WARNING: channel count MISMATCH — model predicts {C_pred} but GT has {C_gt} channels\n')
        run_model._shapes_printed = True

    seg_bev_e_round = torch.sigmoid(seg_bev_e).round()
    save_bev_viz(seg_bev_e, seg_bev_g, valid_bev_g, drivable_bev_g)
...
def main(
        ...
        device_ids=[4,5,6,7],
        # sanity / debug
        use_cpu=False,   # force CPU to rule out MPS numerical issues
        n_steps=0,       # stop after this many steps (0 = full dataset)
):
    B = batch_size
    if use_cpu:
    ...
    if n_steps > 0:
        max_iters = min(max_iters, n_steps)
    ...
    if device.type == 'cuda':
        ...
        model = _SingleDeviceWrapper(model)
