...
# Module-level cache so NuScenesMap JSON is only parsed once per location per process.
_NUSC_MAP_CACHE = {}

def get_drivable_area_mask(nusc, rec, dataroot, Z=200, X=200,
                            XMIN=-50.0, XMAX=50.0, ZMIN=-50.0, ZMAX=50.0):
    """Rasterise nuScenes drivable_area polygons into a (Z, X) uint8 binary mask.

    Returns 1 where the BEV cell lies on a drivable surface, 0 elsewhere.
    Coordinate convention matches get_seg_bev: axis-0 = Z = forward, axis-1 = X = lateral-right.
    """
    ep = nusc.get('ego_pose',
                  nusc.get('sample_data', rec['data']['LIDAR_TOP'])['ego_pose_token'])

    # Map lookup — cache by location to avoid re-parsing JSON every sample.
    # Sentinel value None means the expansion JSON was not found for this location.
    ...
    if location not in _NUSC_MAP_CACHE:
        try:
            ...
            _NUSC_MAP_CACHE[location] = None  # expansion JSONs not downloaded

    nusc_map = _NUSC_MAP_CACHE[location]
    if nusc_map is None:
        return np.zeros((Z, X), dtype=np.uint8)

    # Ego heading in global frame (used by get_local_map to rotate polys into ego frame).
    rot_mat = Quaternion(ep['rotation']).rotation_matrix
    heading = np.arctan2(rot_mat[1, 0], rot_mat[0, 0])
    center = np.array([ep['translation'][0], ep['translation'][1],
                       np.cos(heading), np.sin(heading)])

    # Fetch drivable_area polys rotated into ego frame (col-0=forward, col-1=left).
    # Use a slightly larger stretch than the BEV extent to avoid clipping at edges.
    stretch = max(abs(XMAX), abs(ZMAX)) + 10.0
    lmap = get_local_map(nusc_map, center, stretch, ['drivable_area'], [])

    mask = np.zeros((Z, X), dtype=np.uint8)
    for poly in lmap.get('drivable_area', []):
        # poly[:, 0] = forward distance (ego X, positive = ahead)
        # poly[:, 1] = lateral-left distance (ego Y, positive = left)
        forward  = poly[:, 0]
        lat_left = poly[:, 1]

        # BEV row (Z axis, forward): row = (forward - ZMIN) / (ZMAX - ZMIN) * Z
        row = (forward - ZMIN) / (ZMAX - ZMIN) * Z
        # BEV col (X axis, lateral-right): negate left → right
        col = (-lat_left - XMIN) / (XMAX - XMIN) * X

        pts_px = np.stack([col, row], axis=1).astype(np.int32)  # (N, 2), cv2 expects (x=col, y=row)
        cv2.fillPoly(mask, [pts_px], 1)

    return mask  # (Z, X) uint8
...
class VizData(NuscData):
    ...
    def get_single_item(self, index, cams, refcam_id=None):
        ...
        drivable_bev = torch.from_numpy(
            get_drivable_area_mask(self.nusc, rec, self.dataroot, Z=self.Z, X=self.X)
        ).unsqueeze(0).float()  # (1, Z, X)

        if self.get_tids:
            return imgs, rots, trans, intrins, lidar0_data, lidar0_extra, lidar_data, lidar_extra, lrtlist, vislist, tidlist_, scorelist, seg_bev, valid_bev, center_bev, offset_bev, size_bev, ry_bev, ycoord_bev, radar_data, egopose, drivable_bev
        else:
    ...
    def __getitem__(self, index):
        ...
        for index_t in self.indices[index]:
            ...
            imgs, rots, trans, intrins, lidar0_data, lidar0_extra, lidar_data, lidar_extra, lrtlist, vislist, tidlist, scorelist, seg_bev, valid_bev, center_bev, offset_bev, size_bev, ry_bev, ycoord_bev, radar_data, egopose, drivable_bev = self.get_single_item(index_t, cams, refcam_id=refcam_id)
            ...
            all_drivable_bev.append(drivable_bev)
        ...
        all_drivable_bev = torch.stack(all_drivable_bev)
        ...
        return all_imgs, all_rots, all_trans, all_intrins, all_lidar0_data, all_lidar0_extra, all_lidar_data, all_lidar_extra, all_lrtlist, all_vislist, all_tidlist, all_scorelist, all_seg_bev, all_valid_bev, all_center_bev, all_offset_bev, all_radar_data, all_egopose, all_drivable_bev
