...
class Vox_util(object):
    def __init__(self, Z, Y, X, scene_centroid, bounds, pad=None, assert_cube=False):
        self.XMIN, self.XMAX, self.YMIN, self.YMAX, self.ZMIN, self.ZMAX = [float(b) for b in bounds]
        B, D = list(scene_centroid.shape)
        self.Z, self.Y, self.X = Z, Y, X

        scene_centroid = scene_centroid.detach().cpu().numpy()
        x_centroid, y_centroid, z_centroid = [float(v) for v in scene_centroid[0]]
    ...
    def get_mem_T_ref(self, B, Z, Y, X, assert_cube=False, device='cpu'):
    ...
    def get_ref_T_mem(self, B, Z, Y, X, assert_cube=False, device='cpu'):
        mem_T_ref = self.get_mem_T_ref(B, Z, Y, X, assert_cube=assert_cube, device=device)
    ...
    def unproject_image_to_mem(self, rgb_camB, pixB_T_camA, camB_T_camA, Z, Y, X, assert_cube=False, xyz_camA=None):
        ...
        if (0):
            ...
            # 5D grid_sample (volumetric) is unsupported on MPS — run on CPU, move result back
            # PyTorch's MPS backend only implements the 4D (spatial) variant; 
            # the 5D (volumetric) form silently returns zeros on MPS, which zeros out every feature voxel and leaves the model blind
            _dev = rgb_camB.device
            values = F.grid_sample(rgb_camB.cpu(), xyz_pixB.cpu(), align_corners=False).to(_dev)
    ...
    def warp_tiled_to_mem(self, rgb_tileB, pixB_T_camA, camB_T_camA, Z, Y, X, DMIN, DMAX, assert_cube=False):
        ...
        xyz_pixB = torch.reshape(xyz_pixB, [B, Z, Y, X, 3])
        # 5D grid_sample (volumetric) is unsupported on MPS — run on CPU, move result back
        _dev = rgb_tileB.device
        values = F.grid_sample(rgb_tileB.cpu(), xyz_pixB.cpu(), align_corners=False).to(_dev)
