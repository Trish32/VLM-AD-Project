...
echo "=== train_v3 PLANNING agent-frame + MAP (planning_only) START $(date) ==="
PYTORCH_ENABLE_MPS_FALLBACK=1 conda run -n simple_bev_vldrive python train_v3.py \
  ...
  --save_dir checkpoints/train_v3_plan3 > train_logs/train_v3_plan3.log 2>&1
echo "=== DONE $(date) ==="
echo "[eval] motion/plan (agent-frame + map)"
PYTORCH_ENABLE_MPS_FALLBACK=1 conda run -n simple_bev_vldrive python sparse4d_vl/tools/eval_motion.py \
  --checkpoint checkpoints/train_v3_plan3/epoch_05.pt --eval-set mini_val 2>&1 | grep -E "MOTION|PLANNING|minADE|minFDE|brier|miss|L2|col"
echo "=== ALL DONE $(date) ==="
