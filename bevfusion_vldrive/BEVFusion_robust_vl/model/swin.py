"""
Swin Transformer (microsoft layout) — pure-PyTorch port matching the official
mmdet3d swin_transformer.py so the bevf_pp checkpoint loads exactly.

The `_SwinTransformer` variant adds the CBNet hooks (cb_feats injection,
del_layers, returning intermediate `tmps`).
"""
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, window_size=7, shift_size=0, mlp_ratio=4.,
        ...
        self.drop_path = nn.Identity()  # eval-only (frozen camera stream)
...
class SwinTransformer(nn.Module):
    """Swin-T with CBNet hooks (cb_feats injection, del_layers, tmps return)."""
    ...
    def __init__(self, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24),
        ...
        self.patch_embed = PatchEmbed(4, 3, embed_dim,
