# Ported verbatim from official QCNet losses/nll_loss.py.
...
class NLLLoss(nn.Module):
    """Regression NLL for the chosen single mode (used on the best-matching mode during
    training). With component_distribution=['laplace','laplace'] it scores x and y
    independently: pred packs [loc_x, loc_y, scale_x, scale_y], target packs [x, y]; for
    each dim i it feeds (loc_i, scale_i) at columns [i, D+i]. Output retains the last-dim
    components (reduction='none') so the caller can mask per future step."""
