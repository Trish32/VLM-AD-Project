"""QCNet map encoder. Faithful port of the official module; PyG ops replaced by
``utils.pyg_compat`` and the HeteroData accessed as a nested dict (single-scene / bs=1)."""
...
class QCNetMapEncoder(nn.Module):
    ...
    def forward(self, data: Dict) -> Dict[str, torch.Tensor]:
        # Shape symbols: Pt = num map points (lane/crosswalk boundary & centerline segments),
        # Pl = num map polygons (lane segments + crosswalks), D = hidden_dim, T = hist steps.
        # The map is encoded once (static), then broadcast over time at the end.
        pos_pt = data['map_point']['position'][:, :self.input_dim].contiguous()   # [Pt, 2]
        orient_pt = data['map_point']['orientation'].contiguous()                 # [Pt]
        pos_pl = data['map_polygon']['position'][:, :self.input_dim].contiguous()  # [Pl, 2]
        orient_pl = data['map_polygon']['orientation'].contiguous()               # [Pl]
        orient_vector_pl = torch.stack([orient_pl.cos(), orient_pl.sin()], dim=-1)  # [Pl, 2]
        ...
        # polygon<->polygon edges = union of (a) topological connectivity from the map
        # (PRED/SUCC/LEFT/RIGHT, type 1-4) and (b) spatial proximity within pl2pl_radius
        # (type 0). merge_edges coalesces duplicates keeping the max type code.
        edge_index_pl2pl = data['map_polygon', 'to', 'map_polygon']['edge_index']  # [2, E] connectivity
        edge_index_pl2pl_radius = radius_graph(x=pos_pl[:, :2], r=self.pl2pl_radius, batch=None,
                                               loop=False, max_num_neighbors=300)  # [2, E] proximity
        ...
        # Each layer: points -> polygon aggregation, then polygon <-> polygon attention.
        for i in range(self.num_layers):
            x_pl = self.pt2pl_layers[i]((x_pt, x_pl), r_pt2pl, edge_index_pt2pl)  # [Pl, D]
            x_pl = self.pl2pl_layers[i](x_pl, r_pl2pl, edge_index_pl2pl)          # [Pl, D]
        # broadcast the static map encoding across all historical steps -> [Pl, T, D]
        x_pl = x_pl.repeat_interleave(repeats=self.num_historical_steps,
                                      dim=0).reshape(-1, self.num_historical_steps, self.hidden_dim)

        return {'x_pt': x_pt, 'x_pl': x_pl}  # point tokens [Pt, D], polygon tokens [Pl, T, D]
